<script lang="ts">
  ...
  let { checked = $bindable(false), thumbProps, ...restProps }: SwitchProps = $props()
</script>

<Switch.Root
  bind:checked
  class={[
    'h-6 w-12 shrink-0 flex items-center rounded-full p-1 shadow/20 transition-[opacity,background-color] duration-200',
...
>
  <Switch.Thumb
    class={[
      'w-4 h-4 data-[state=checked]:translate-x-6 data-[state=unchecked]:translate-x-0 shrink-0 rounded-full 750 transition-[translate,background-color] duration-200',
      't-dark:bg-zinc-850',
      't-light:bg-zinc-100',
      't-rust:bg-rust-850',
      't-midnight:bg-gray-850'
    ]}
    {...thumbProps}
  />
</Switch.Root>
