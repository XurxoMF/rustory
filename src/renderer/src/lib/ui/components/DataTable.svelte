      ...
      <tr class={['hover:bg-zinc-800', 't-light:hover:bg-zinc-300']}>
        ...
        {#each columns as col (col.key)}
          <th class={['text-start p-2 whitespace-nowrap']}>
            {#if col.sort}
              <button type="button" onclick={() => handleSort(col)} class="flex items-center gap-2 cursor-pointer">
                <span>{col.label}</span>
    ...
    <tbody class={['divide-y', 'divide-zinc-800', 't-light:divide-zinc-300']}>
      {#each sortedRows as row (row.id)}
        <tr
          ...
          class={['data-[selected=true]:bg-zinc-800/50 hover:bg-zinc-800', 't-light:hover:bg-zinc-300']}
        >
          {#if selectable}
