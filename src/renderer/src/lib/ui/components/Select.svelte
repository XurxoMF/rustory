...
<Select.Root bind:value={value as never} onValueChange={onValueChange as never} {allowDeselect} {...restProps}>
  ...
  <Select.Portal to="#portal">
    <Select.Content
      sideOffset={4}
      class={[
        'max-h-(--bits-select-content-available-height) w-(--bits-select-anchor-width) min-w-(--bits-select-anchor-width) z-50 flex flex-col backdrop-blur-xs rounded-sm shadow/30 outline-none transition-all',
