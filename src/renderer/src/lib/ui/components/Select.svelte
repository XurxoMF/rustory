...
<Select.Root bind:value={value as never} {allowDeselect} {...restProps}>
  ...
  >
    ...
    {:else}
      <span class="text-current/30 leading-tight">{placeholder}</span>
    {/if}
  ...
  <Select.Portal to="#portal">
    <Select.Content
      sideOffset={4}
      class={[
        'max-h-[calc(var(--bits-select-content-available-height)-0.5rem)] w-(--bits-select-anchor-width) z-300 flex flex-col backdrop-blur-xs rounded-sm shadow-xl outline-none transition-all @container',
        'inset-ring-2',
