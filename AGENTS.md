...
- Os ZIP de versións entregados pola API de Rustory xa están normalizados para que, despois da extracción, o executable correspondente estea na raíz do directorio da versión. O cliente non debe reconstruír nin buscar layouts internos dos paquetes orixinais de Vintage Story, como bundles `.app`; esa normalización pertence á API.
- Na raíz normalizada, Windows require exactamente `Vintagestory.exe`; Linux e macOS requiren exactamente `Vintagestory`, sen fallback a `.exe`. Se o nome correspondente non existe, a versión non é unha instalación válida. `VSVersion.getExecutable()` obtén a plataforma desde `Info` por defecto e rexeita plataformas non soportadas. A arquitectura macOS xa se resolveu ao seleccionar o artefacto e non cambia o nome do executable.
- A limpeza de staging e descargas temporais execútase sempre, reintenta unha vez os fallos de filesystem e non pode ocultar o erro orixinal nin converter unha instalación xa publicada nun fallo. Un fallo permanente de limpeza debe quedar rexistrado no log.
- Os formularios gardan primeiro a instancia e logo usan `Data.queueVsVersionInstallation()`. Unha versión nova rexístrase inmediatamente con estado `NOT_INSTALLED` e instálase nunha tarefa de fondo xestionada por `Data`; isto permite redirixir sen bloquear o formulario. Se falla, a versión e a instancia permanecen rexistradas e a versión volve a `NOT_INSTALLED`.
- Antes dun lanzamento, usa `Data.ensureVsVersionInstalled()`: devolve inmediatamente unha versión xa instalada, agarda unha instalación que estea en curso ou inicia unha nova. Comparte a mesma Promise ca cola non bloqueante para evitar duplicados. Se a instalación falla, propaga o erro ao lanzamento e conserva a versión rexistrada como `NOT_INSTALLED`.
- Durante o arranque, cada ruta de versión rexistrada valídase illadamente mediante `Data.loadVsVersion()`. Un executable ausente, unha versión corrupta ou unha instalación interrompida non abortan a carga de `Data`: a versión consérvase como `NOT_INSTALLED` para poder reinstalala posteriormente.
...
- `VSLaunchSpec` é un obxecto de datos inmutable, non unha clase nin un singleton. `VSInstance.createLaunchSpec()` créao antes de iniciar o xogo coa versión, executable, directorio de traballo, directorio de datos, argumentos e variables. O control do proceso e os futuros métodos `play()` e `stop()` pertencen a `VSInstance`.
...
- Distinguir rexistro e dispoñibilidade: unha versión pode persistirse antes de estar instalada, pero só pode ter estado `INSTALLED` tras completar checksum, extracción e validación. As instancias que a usan deben conservarse aínda que a instalación falle.
- Non iniciar `VSVersion.install()` directamente como fire-and-forget desde un compoñente. As instalacións non bloqueantes deben pasar por `Data.queueVsVersionInstallation()`, que conserva a Promise, evita duplicados e consume os erros da tarefa de fondo.
- As operacións que necesiten a versión dispoñible antes de continuar, como Play, deben usar e agardar `Data.ensureVsVersionInstalled()`; non deben observar o estado nun bucle nin iniciar unha segunda instalación pola súa conta.
